...
let envacc env n =
  ...
  | Rec_closure (_o, t, blk, i) ->
    blk.(n - ((2 * Array.length t) + 1 - (2 * i)))
...
let eval globals code cfuns =
  ...
  let rec box_env env =
    Obj.repr (fun arg ->
      if arg == getenv_key then Obj.repr env
      else
        let accu = ref (box_env env) in
        Astack.push stack (Obj.repr 0);
        (* extra_args *)
        Astack.push stack (Obj.repr No_closure);
        (* env        *)
        Astack.push stack (Obj.repr stop_pc);
        (* pc -> STOP *)
        Astack.push stack arg;
        (* the arg    *)
        run (ptr_of_env env) env accu;
        !accu )
  and unbox_env f = (Obj.obj f : Obj.t -> env) getenv_key
      ...
      while true do
        try
          while true do
            match code.(!pc) with
            | Instr.ACC0 ->
              accu := Astack.acc stack 0;
              incr pc
            | Instr.ACC1 ->
              accu := Astack.acc stack 1;
              incr pc
            | Instr.ACC2 ->
              accu := Astack.acc stack 2;
              incr pc
            | Instr.ACC3 ->
              accu := Astack.acc stack 3;
              incr pc
            | Instr.ACC4 ->
              accu := Astack.acc stack 4;
              incr pc
            | Instr.ACC5 ->
              accu := Astack.acc stack 5;
              incr pc
            | Instr.ACC6 ->
              accu := Astack.acc stack 6;
              incr pc
            | Instr.ACC7 ->
              accu := Astack.acc stack 7;
              incr pc
            | Instr.ACC n ->
              accu := Astack.acc stack n;
              incr pc
            | Instr.PUSH | Instr.PUSHACC0 ->
            ...
            | Instr.MAKEBLOCK (tag, sz) ->
              ...
              for i = 1 to sz - 1 do
                Obj.set_field blk i (Astack.pop stack)
              done;
              accu := blk;
              incr pc
            ...
            | Instr.MAKEFLOATBLOCK sz ->
              ...
              incr pc
            | Instr.GETFIELD0 ->
              accu := Obj.field !accu 0;
              incr pc
            | Instr.GETFIELD1 ->
              accu := Obj.field !accu 1;
              incr pc
            | Instr.GETFIELD2 ->
              accu := Obj.field !accu 2;
              incr pc
            | Instr.GETFIELD3 ->
              accu := Obj.field !accu 3;
              incr pc
            | Instr.GETFIELD n ->
              accu := Obj.field !accu n;
              incr pc
            | Instr.GETFLOATFIELD ind ->
              accu := Obj.repr (Obj.double_field !accu ind);
              incr pc
            | Instr.SETFIELD0 ->
              Obj.set_field !accu 0 (Astack.pop stack);
              accu := unit;
              incr pc
            | Instr.SETFIELD1 ->
              Obj.set_field !accu 1 (Astack.pop stack);
              accu := unit;
              incr pc
            | Instr.SETFIELD2 ->
              Obj.set_field !accu 2 (Astack.pop stack);
              accu := unit;
              incr pc
            | Instr.SETFIELD3 ->
              Obj.set_field !accu 3 (Astack.pop stack);
              accu := unit;
              incr pc
            | Instr.SETFIELD n ->
              Obj.set_field !accu n (Astack.pop stack);
              accu := unit;
              incr pc
            ...
            | Instr.CHECK_SIGNALS -> incr pc
            | Instr.C_CALL1 idx ->
              accu := (Obj.obj cfuns.(idx) : Obj.t -> Obj.t) !accu;
              incr pc
            | Instr.C_CALL2 idx ->
              accu :=
                (Obj.obj cfuns.(idx) : Obj.t -> Obj.t -> Obj.t)
                  !accu (Astack.pop stack);
              incr pc
            | Instr.C_CALL3 idx ->
              accu :=
                (Obj.obj cfuns.(idx) : Obj.t -> Obj.t -> Obj.t -> Obj.t)
                  !accu (Astack.acc stack 0) (Astack.acc stack 1);
              Astack.popn stack 2;
              incr pc
            | Instr.C_CALL4 idx ->
              accu :=
                (Obj.obj cfuns.(idx) : Obj.t -> Obj.t -> Obj.t -> Obj.t -> Obj.t)
                  !accu (Astack.acc stack 0) (Astack.acc stack 1)
                  (Astack.acc stack 2);
              Astack.popn stack 3;
              incr pc
            | Instr.C_CALL5 idx ->
              accu :=
                ( Obj.obj cfuns.(idx)
                  ...
                  !accu (Astack.acc stack 0) (Astack.acc stack 1)
                  (Astack.acc stack 2) (Astack.acc stack 3);
              Astack.popn stack 4;
              incr pc
            ...
            | Instr.ADDINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) + (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.SUBINT ->
              accu :=
                Obj.repr
            ...
            | Instr.MODINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) mod (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.ANDINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) land (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.ORINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) lor (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.XORINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) lxor (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.LSLINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) lsl (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.LSRINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) lsr (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.ASRINT ->
              accu :=
                Obj.repr
                  ((Obj.obj !accu : int) asr (Obj.obj (Astack.pop stack) : int));
              incr pc
            | Instr.EQ ->
            ...
            | Instr.GETMETHOD ->
              accu :=
                Obj.field
                  (Obj.field (Astack.acc stack 0) 0)
                  (Obj.obj !accu : int);
              incr pc
            | Instr.BEQ (n, ptr) ->
            ...
            | Instr.UGEINT ->
              ...
              incr pc
            | Instr.BULTINT (n, ptr) ->
              if n + min_int < (Obj.obj !accu : int) + min_int then pc := ptr
              else incr pc
            | Instr.BUGEINT (n, ptr) ->
              if n + min_int >= (Obj.obj !accu : int) + min_int then pc := ptr
              else incr pc
            ...
            | Instr.GETDYNMET ->
              accu :=
                find_object_method (Astack.acc stack 0) (Obj.obj !accu : int);
              incr pc
