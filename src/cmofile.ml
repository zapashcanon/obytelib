...
module Legacy_V008 : sig
  ...
  let import =
    let import_constant cst =
      ...
      | Const_nativeint n -> T.Const_nativeint n
    in
    ...
    let rec import_structured_constant sc =
      ...
      | Const_block (tag, scs) ->
        T.Const_block (tag, List.map import_structured_constant scs)
      | Const_float_array floats -> T.Const_float_array floats
      | Const_immstring str -> T.Const_immstring str
    in
    ...
    let import_reloc_info reloc_info =
      ...
      | Reloc_primitive prim -> T.Reloc_primitive prim
    in

    let import_cu_reloc cu_reloc =
      List.map
        (fun (reloc_info, n) -> (import_reloc_info reloc_info, n))
        cu_reloc
    in
  ...
  let export =
    let export_constant cst =
      ...
      | T.Const_nativeint n -> Const_nativeint n
    in
    ...
    let rec export_structured_constant sc =
      ...
      | T.Const_block (tag, scs) ->
        Const_block (tag, List.map export_structured_constant scs)
      | T.Const_float_array floats -> Const_float_array floats
      | T.Const_immstring str -> Const_immstring str
    in
    ...
    let export_reloc_info reloc_info =
      ...
      | T.Reloc_primitive prim -> Reloc_primitive prim
    in

    let export_cu_reloc cu_reloc =
      List.map
        (fun (reloc_info, n) -> (export_reloc_info reloc_info, n))
        cu_reloc
    in
...
let reloc cmo =
  let value_of_constant c =
    ...
    | Const_nativeint n -> Value.Nativeint n
  in

  let rec value_of_structured_constant sc =
    match sc with
    | Const_base c -> value_of_constant c
    | Const_pointer ptr -> Value.Int ptr
    | Const_block (tag, fields) ->
      let fields =
        Array.of_list (List.map value_of_structured_constant fields)
      in
    ...
    | Const_immstring str -> Value.String str
  in
  ...
  List.iter
    (fun (reloc_info, pos) ->
      match reloc_info with
      | Reloc_literal sc ->
        Hashtbl.add literals (pos / 4)
          (value_of_structured_constant sc, !data_ind);
        incr data_ind
  ...
  let find_global pos =
    ...
    with Not_found -> snd (Hashtbl.find globals pos)
  in
  let find_prim pos = snd (Hashtbl.find primitives pos) in
  Array.iteri
    (fun instr_ind instr ->
      let word_count =
        let cnt = ref 0 in
      ...
      let new_instr =
        try
          ...
          | Instr.PUSHGETGLOBAL _ ->
            Instr.PUSHGETGLOBAL (find_global (!code_pos + 1))
          | Instr.GETGLOBALFIELD (_, n) ->
            Instr.GETGLOBALFIELD (find_global (!code_pos + 1), n)
          | Instr.PUSHGETGLOBALFIELD (_, n) ->
            Instr.PUSHGETGLOBALFIELD (find_global (!code_pos + 1), n)
...
let print =
  ...
  let print_constant oc c =
    ...
    | Const_string (s, o) ->
      fprintf oc "Const_string (%S, %a)" s print_string_option o
  ...
  let rec print_structured_constant oc sc =
    ...
    | Const_block (n, scs) ->
      fprintf oc "Const_block (%d, %a)" n
        (print_list print_structured_constant)
        scs
    | Const_float_array fs ->
      fprintf oc "Const_float_array %a"
        (print_list (fun oc s -> fprintf oc "%S" s))
        fs
  ...
  let print_reloc_info oc ri =
    ...
    | Reloc_primitive s -> fprintf oc "Reloc_primitive %S" s
  in
  ...
  let print_compilation_unit oc
    ...
    fprintf oc
      ...
      (print_vlist 2 (fun oc (ri, n) ->
         fprintf oc "(%a, %d)" print_reloc_info ri n ) )
      cu_reloc
      (print_vlist 2 (fun oc (s, d) ->
         fprintf oc "(%S, %s)" s
           ( match d with
           | None -> "None"
           | Some d -> "Some \"" ^ Digest.to_hex d ^ "\"" ) ) )
      cu_imports
      (print_vlist 2 Ident.print)
      cu_required_globals
      (print_vlist 2 (fun oc s -> fprintf oc "%S" s))
...
let read file_name =
  ...
  try
    let version =
      ...
      | None ->
        failwith
          (Printf.sprintf "unexpected magic string for a .cmo file: %S" str)
    in
    ...
    let unit =
      ...
      | Version.V011 | Version.V022 | Version.V023 | Version.V025 | Version.V026
      | Version.V027 | Version.V028 | Version.V029 | Version.V030 | Version.V031
        ->
...
let write file_name { version; unit; code } =
  ...
  let marshaled_unit =
    ...
    | Version.V027 | Version.V028 | Version.V029 | Version.V030 | Version.V031
      ->
      Marshal.to_string unit []
  in
  let oc =
    try open_out file_name
    with _ -> fail "fail to open file %S for writting" file_name
  in
  try
  ...
  | exn ->
    close_out oc;
    fail "fail to write cmo file %S (internal error: %s)" file_name
      (Printexc.to_string exn)
