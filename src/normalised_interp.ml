...
let envacc env n =
  ...
  | Rec_closure (ptrs, blk, i) ->
    blk.(n - ((2 * Array.length ptrs) - 1 - (2 * i)))
        ...
        try
          while true do
            ...
            | Normalised_instr.APPLY n ->
              if n < 4 then (
                let args =
                  Array.init n (fun i -> Astack.acc stack (n - i - 1))
                in
                Astack.popn stack n;
                Astack.push stack (Obj.repr !extra_args);
                Astack.push stack (Obj.repr !env);
            ...
            | Normalised_instr.SETBYTESCHAR ->
              ...
              accu := unit;
              incr pc
            | Normalised_instr.BRANCH ptr -> pc := ptr
            ...
            | Normalised_instr.POPTRAP ->
              ...
              Astack.popn stack 4;
              incr pc
            | Normalised_instr.RAISE | Normalised_instr.RERAISE
            | Normalised_instr.RAISE_NOTRACE ->
              raise (Obj.obj !accu : exn)
            | Normalised_instr.CHECK_SIGNALS -> incr pc
            ...
            | Normalised_instr.GETDYNMET ->
              ...
              incr pc
            | Normalised_instr.STOP -> raise Stop
          done
        with
