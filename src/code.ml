...
let kosaraju_gen get_nexts instrs =
  ...
  in
  Array.iteri
    (fun ind -> List.iter (fun ind' -> preds.(ind') <- ind :: preds.(ind')))
    nexts;
...
let kosaraju instrs = kosaraju_gen Instr.get_nexts instrs
...
let print_gen get_ptrs get_nexts bprint_instr data symb prim oc instrs =
  ...
  Array.iter
    (fun instr -> List.iter (fun ptr -> labels.(ptr) <- 0) (get_ptrs instr))
    instrs;
  begin
    let cnt = ref 1 in
  ...
  for i = 0 to instr_nb - 1 do
    ...
    while Buffer.length buf < margin_sz do
      Buffer.add_char buf ' '
    done;
...
let normalise_envaccs version instrs =
  match version with
  | Version.V008 | Version.V010 | Version.V011 | Version.V022 | Version.V023
  ...
  | Version.V029 | Version.V030 | Version.V031 ->
    ...
    Array.iter
      (fun instr ->
        match instr with
        | CLOSURE (_, ptr) -> Hashtbl.add code_ptr_nb colors.(ptr) 1
        | CLOSUREREC (_, _, o, t) ->
          let delta = 1 + Array.length t in
          Hashtbl.add code_ptr_nb colors.(o) delta;
          Array.iteri
            (fun i ptr -> Hashtbl.add code_ptr_nb colors.(ptr) (delta - i - 1))
            t
        | _ -> () )
      instrs;
...
let read version index ic =
  ...
  begin
    ...
    with End_of_file -> fail "truncated CODE section"
  end;
...
let write version oc code =
  ...
  let write_code _ind w =
    ...
    output_byte oc ((w asr 24) land 0xFF)
  in
...
let size version code =
  ...
  Array.iter
    (fun instr ->
      Instr.write version
        (fun _word -> incr counter)
        (fun _delta _ptr -> incr counter)
        instr )
    code;
  !counter * 4
