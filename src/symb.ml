...
let tree_of_table table =
  let aggr_idents acc ident_opt =
    match ident_opt with None -> acc | Some ident -> ident :: acc
  in
  ...
  let rec make_tree i j =
    if i >= j then Tree.Empty
    else
      let m = (i + j) / 2 in
      let left = make_tree i m
      and right = make_tree (m + 1) j in
      let h = max (Tree.height left) (Tree.height right) + 1 in
      Tree.Node (left, sorted_tbl.(m), m, right, h)
  in
...
let write version oc table =
  ...
  let marshaled_value = { num_cnt; num_tbl = tree } in
