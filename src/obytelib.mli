...
module Version : sig
  (** Bytecode versions corresponding to magic strings: "Caml1999X008",
      "Caml1999X010", ..., "Caml1999X031" *)
  type t =
    | V008
    | V010
    | V011
  ...
  (** Conversion of magic string to version. Raise a Failure if the parameter is
      an unknwon magic string *)
  val of_magic : string -> t

  (** Read version from a bytecode file. Automatically move to the end of the
      file. Raise a Failure if the file is too short or the magic string invalid
  *)
...
module Index : sig
  (** Index entry containning the section name, the offset from the beginning of
      the file (in bytes), and the length of the section (in bytes) *)
  type entry =
    { section : Section.t
    ; offset : int
    ; length : int
  ...
  (** Find a section in an index. Raise a Failure if the requested section is
      not found. Return a tuple (offset, length) where offset is the offset of
      the section from the beginning of the file in bytes, and length is the
...
(** Manipulation of the binary data stored between the VM command and the
...
module Dlpt : sig
  ...
  (** Read the DLPT section from a bytecode file. If it does not appear, return
      an empty array *)
  val read : Index.t -> in_channel -> t
...
(** Management of the DLLS section containing the names of needed dynamic
    libraries *)
module Dlls : sig
  ...
  (** Read the DLLS section from a bytecode file. If it does not appear, return
      an empty array *)
  val read : Index.t -> in_channel -> t
...
(** Management of the CRCS section containing module names and MD5 hash of the
...
module Crcs : sig
  ...
  (** Read the CRCS section from a bytecode file. Return an empty array if the
      CRCS section does not appear in the bytecode file *)
  val read : Index.t -> in_channel -> t
...
(** Management of the SYMB section containing meta-data about global table (see
    the DATA section) *)
module Symb : sig
  ...
  (** Read the SYMB section from a bytecode file. Return empty if no SYMB
      section found. Check compatibility with the given version. *)
  val read : Version.t -> Index.t -> in_channel -> t

  (** Write the SYMB section in a bytecode file. Check compatibility with the
      given version. *)
...
module Dbug : sig
  ...
  (** Read the DBUG section from an bytecode file. Return an empty list if no
      DBUG section found *)
  val read : Index.t -> in_channel -> t
...
module Value : sig
  ...
  (** Convert an Obj.t (usually comming from a cell of the DATA section) to a
      structured OCaml tree *)
  val of_obj : Obj.t -> t

  (** [make_to_obj ()] creates a converter of values to Obj.t that factorize
      int32, in64, nativeints and floats. Usefull for convert a table of Obj.t
      that share immutable blocks *)
  val make_to_obj : unit -> t -> Obj.t
end

(** Reading, writing, pretty-printing and management of the DATA section
    containing the array of global data *)
module Data : sig
  ...
  (** Read the DATA section from a bytecode file. Raise a Failure if the DATA
      seciton does not exists *)
  val read : Index.t -> in_channel -> t
  ...
  (** Replace the first cells of the DATA array containning pointers to standard
      exceptions (Failure, Stack_overflow, etc.) to share them with the current
      process exceptions *)
  val fix_std_exceptions : Obj.t array -> unit
end

(** Reading, writing, pretty-printing and management of the PRIM section
    containing the mapping of C primitive names to C_CALL ids *)
module Prim : sig
  (** Type of table of primitive names. Indexes in the table correspond to
      C_CALL ids and contained strings are C function names *)
  type t = string array
...
(** Low-level bytecode instruction management. Each constructor correspond
    directly to a VM instruction. To use a normalised representation of bytecode
    instructions, please see the Normalised_* modules *)
module Instr : sig
  (** Type representing the 148 bytecode instructions as they are defined for
      the V010 bytecode version *)
  type t =
    | ACC0
    | ACC1
    | ACC2
  ...
  (** Get the code pointers from an instruction. Code pointers are indexes in
      the table of instructions (see the Code module) *)
  val get_ptrs : t -> int list

  (** Get the list of instruction indexes that may be executed after the
      execution of the given one. The first integer argument should be the index
      in the code of the given instruction. *)
  val get_nexts : int -> t -> int list
end

(** Reading, writting and pretty-printing of the CODE section *)
module Code : sig
  (** Type representing the imported CODE section: an array of instructions.
      Code pointers from instructions are indexes in the table of instructions
  *)
  type t = Instr.t array

  (** Group instructions into connected components, separating function bodies
      and top-level code. Return an integer array of the same size of the given
      code. Two instructions are in the same connected component iff they
      correspond to the same integer in the returned array *)
  val kosaraju : t -> int array

  (** Pretty-print code *)
  val print : Data.t -> Symb.t -> Prim.t -> out_channel -> t -> unit

  (** Read the CODE section from a bytecode file. If it does not appear, raise a
      Failure *)
  val read : Version.t -> Index.t -> in_channel -> t
...
module Cmofile : sig
  ...
  type compilation_unit =
    ...
    ; cu_required_globals : Ident.t list
        (* Compilation units whose initialization side effects must occur before this one. *)
...
(** Normalised version of the bytecode instruction set. Remove redondancies like
    ACC0/ACC 0 or PUSHACC/PUSH+ACC, group operators, and simplify/prepare some
    instruction parameters (see CLOSUREREC and SWITCH) *)
module Normalised_instr : sig
  ...
  (** Type for binary operators (except comparisons, see compop) giving accu and
      stack top as parameters, poping the top of the stack and storing the
      result in the accu without side effect *)
  type binop =
    | ADD
    | SUB
    | MUL
  ...
  (** Comparison operators used for interger comparison instructions and
      conditionnal branchs based on a comparison *)
  type compop =
    | EQ
    | NEQ
    | LT
  ...
  (** Return code pointers referenced by the given instructions. Code pointers
      ar indexes in the code represented by an array of instructions *)
  val get_ptrs : t -> int list
...
module Normalised_code : sig
  (** Type of a normalised code segment: an array of normalised instructions.
      Code pointers contained in instructions are indexes in the embedding code
      array *)
  type t = Normalised_instr.t array

  (** Group instructions into connected components, separating function bodies
      and top-level code. See the similar function Code.kosaraju for more
      details *)
  val kosaraju : t -> int array
...
(** Bytecode interpreter written in OCaml supporting callbacks, closure exchange
    and cross exceptions raise/catch *)
module Interp : sig
  (** [eval globals code prim] interpretes the given bytecode using the global
      array [globals] and the primitive map [prim]. If an exception escapes from
      the bytecode, it is raised through eval *)
  val eval : Obj.t array -> Code.t -> Obj.t array -> unit

  (** Interprete the given bytecode file in the current environment. Support
      callbacks, closure exchange and cross exceptions raise/catch. If an
      exception escapes from the bytecode, it is raised through eval. Be aware
      of side effects provided by the C runtime which are shared between the
      current program and the bytecode program. For example: the bytecode
      at_exit function overload the current at_exit *)
...
module Normalised_interp : sig
  ...
  (** Same as Interp.eval_bytecode but start to convert standard bytecode into
      normalised code before interpreting it *)
  val eval_bytefile : Bytefile.t -> unit
end
